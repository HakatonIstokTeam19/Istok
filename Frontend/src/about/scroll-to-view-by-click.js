      ...
      entries.forEach((entry) => {
        if (entry.isIntersecting) {
          ...
          controls.forEach((control) => {
            const controlCircle = control.querySelector(
              ".slide-control__circle"
            );
            if (
              control.getAttribute("data-related-section-id") === sectionId || (sectionId === "values_2" && control.getAttribute("data-related-section-id") === "values")
            ) {
              controlCircle.classList.add("slide-control__circle_active");
            } else {
              controlCircle.classList.remove(
                "slide-control__circle_active"
              );
            }
          });
        }
